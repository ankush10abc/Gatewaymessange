...
class ApiService {
  ...
  Future<Map<String, dynamic>> batchSendMessages(List<Map<String, dynamic>> messages) async {
    debugPrint('API Call: POST $baseUrl/api/messages/batch-send - Count: ${messages.length}');
    final response = await _dio.post(
      '/api/messages/batch-send',
      data: {'messages': messages},
    );
    debugPrint('API Response: Sent ${response.data['data']['total_sent']}, Failed ${response.data['data']['total_failed']}');
    return response.data;
  }
  ...
  Future<Map<String, dynamic>> syncMessages({
    ...
    debugPrint('API Call: GET $baseUrl/api/messages/sync - Params: $params');
    final response = await _dio.get('/api/messages/sync', queryParameters: params);
    debugPrint('API Response: ${response.data['data']['messages'].length} new messages');
    return response.data;
  }

  /// API 3: Incremental Chat List Sync - Fetch only changed chats since last sync
  Future<Map<String, dynamic>> syncChats({DateTime? since, int limit = 50}) async {
    final params = {
      if (since != null) 'since': since.toIso8601String(),
      'limit': limit,
    };
    debugPrint('API Call: GET $baseUrl/api/chats/sync');
    final response = await _dio.get('/api/chats/sync', queryParameters: params);
    final data = response.data['data'];
    debugPrint('API Response: ${data['new_chats'].length} new, ${data['updated_chats'].length} updated');
    return response.data;
  }
  ...
  Future<Map<String, dynamic>> batchMarkAsRead({
    ...
    debugPrint('API Call: POST $baseUrl/api/messages/batch-read - Count: ${messageIds.length}');
    final response = await _dio.post('/api/messages/batch-read', data: request);
    debugPrint('API Response: ${response.data['data']['marked_count']} messages marked as read');
    return response.data;
  }

  /// API 5: Queue Status - Check pending/failed message counts
  Future<Map<String, dynamic>> getQueueStatus() async {
    debugPrint('API Call: GET $baseUrl/api/sync/queue-status');
    final response = await _dio.get('/api/sync/queue-status');
    debugPrint('API Response: ${response.data['data']['pending_messages']} pending');
    return response.data;
  }
  ...
  Future<Map<String, dynamic>> checkAppVersion({
    ...
    debugPrint('API Call: GET $baseUrl/api/app/version-check - v$currentVersion');
    final response = await _dio.get('/api/app/version-check', queryParameters: params);
    final updateAvailable = response.data['data']['update_available'];
    debugPrint('API Response: Update available: $updateAvailable');
    return response.data;
  }
}

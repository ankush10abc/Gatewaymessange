...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _checkForUpdate();
      // debugPrint('messengerKey.currentState: ${Helpers.instance.messengerKey.currentState}');
      // Helpers.instance.showSnackBar('Ready', 'Snackbar system online');
    });
...
var fcmToken = '';
  ...
  Future<void> _handleLogin() async {
    ...
    // Capture notifier before any await — safe even if widget disposes later
    ...
    // Use the pre-captured notifier — never touch ref after an await
    ...
    if (success) {
      // Go to splash so it runs prefetch + animation before navigating to home
      context.go('/splash');
    } else {
      // Extract error from current auth state and show snackbar
      final error = ref.read(authProvider).error;
      if (error != null && error.isNotEmpty) {
        Helpers.instance.showSnackBar(error,error
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Update inline error widget when auth state has error

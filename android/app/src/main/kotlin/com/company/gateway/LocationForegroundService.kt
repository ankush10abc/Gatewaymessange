...
class LocationForegroundService : Service() {
    ...
    companion object {
        ...
        // Called on login — starts the foreground service only if location permission is granted
        fun start(context: Context) {
            // Guard: only start if at least coarse or fine location permission is granted
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Keeps app active for real-time messaging"
                setShowBadge(false)
            }
            val nm = getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
